# [Lynx](https://getlynx.io) Block Crawler

The Lynx Block Crawler is a PHP application that provides real-time blockchain inspection tools to the user. In addition to being able to research discreet transactions, users can inspect blocks and addresses. The interface also provides FAQ and News feeds from the Lynx website plus widgets from various partner sites. The Block Crawler is installed by default with LynxCI and LynxCI ISO for Raspberry Pi.
